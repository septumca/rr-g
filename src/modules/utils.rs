...
pub fn is_point_in_rect(point: &bevy::prelude::Vec2, rect_origin: &bevy::prelude::Vec3, rect_hal_size: f32) -> bool {
    point.x > (rect_origin.x - rect_hal_size) &&
        point.x < (rect_origin.x + rect_hal_size) &&
        point.y > (rect_origin.y - rect_hal_size) &&
        point.y < (rect_origin.y + rect_hal_size)
}
